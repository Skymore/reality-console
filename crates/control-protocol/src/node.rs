...
/// Controller-owned lifecycle visible to the enrolled node itself.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub enum NodeLifecycleState {
    /// Enrollment succeeded but an operator has not approved the node.
    Pending,
    /// The operator approved the node for desired state and endpoint checks.
    Active,
}

impl NodeLifecycleState {
    /// Returns the canonical transcript value.
    #[must_use]
...
/// Controller-owned progress for one currently reported endpoint candidate.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub enum EndpointReadiness {
    /// No external TCP preflight has completed.
    Pending,
    /// An external TCP preflight currently holds a finite claim.
    Checking,
    /// External TCP connected, but the end-to-end protocol canary is pending.
    TcpReachable,
    /// The latest external TCP preflight could not connect.
    TcpUnreachable,
    /// The controller has complete current end-to-end verification evidence.
    Verified,
}
...
/// Redacted controller status for one current endpoint candidate.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase", deny_unknown_fields)]
pub struct NodeEndpointStatus {
    /// Candidate identity originally generated by this Node Host.
    pub endpoint_id: EndpointId,
    /// Current controller-owned verification progress.
    pub readiness: EndpointReadiness,
    /// Completion time of the latest relevant external check.
    pub last_checked_at: Option<Timestamp>,
    /// Stable secret-free failure code for `tcpUnreachable`.
    pub error_code: Option<String>,
}

impl NodeEndpointStatus {
    fn validate(&self) -> Result<(), ProtocolValidationError> {
        let consistent = match self.readiness {
            EndpointReadiness::Pending | EndpointReadiness::Checking => {
                self.last_checked_at.is_none() && self.error_code.is_none()
            }
            EndpointReadiness::TcpReachable | EndpointReadiness::Verified => {
                self.last_checked_at.is_some() && self.error_code.is_none()
            }
            EndpointReadiness::TcpUnreachable => {
                self.last_checked_at.is_some() && self.error_code.is_some()
            }
        };
        if !consistent {
            return Err(ProtocolValidationError::new(
                ValidationCode::InconsistentState,
                "endpoints.readiness",
                "endpoint readiness must agree with its check time and error code",
            ));
        }
        if let Some(error_code) = &self.error_code {
            validate_short_text(error_code, 128, "endpoints.errorCode")?;
            if !error_code
                .bytes()
                .all(|byte| byte.is_ascii_lowercase() || byte.is_ascii_digit() || byte == b'_')
            {
                return Err(ProtocolValidationError::new(
                    ...
                    "endpoints.errorCode",
                    "endpoint error code must use lowercase snake case",
                ));
            }
        }
        Ok(())
...
/// Controller-signed self-status bound to one accepted heartbeat generation.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase", deny_unknown_fields)]
pub struct NodeHeartbeatStatus {
    /// Signed self-status schema version.
    pub schema_version: u16,
    /// Node this status describes.
    pub node_id: NodeId,
    /// Exact heartbeat generation that requested this status.
    pub heartbeat_generation: SequenceNumber,
    /// Controller observation time.
    pub observed_at: Timestamp,
    /// Current controller-owned node lifecycle.
    pub lifecycle: NodeLifecycleState,
    /// Current endpoint candidates sorted by endpoint identity.
    pub endpoints: Vec<NodeEndpointStatus>,
    /// Controller key that signed this document.
    pub signing_key_id: SigningKeyId,
    /// Controller instance that owns this node registration.
    pub controller_instance_id: ControllerInstanceId,
}

impl NodeHeartbeatStatus {
    /// Validates identity binding, schema, generation, ordering, and endpoint consistency.
    ///
    /// # Errors
    ///
    /// Returns an error for an unsupported schema, wrong identity binding,
    /// non-positive generation, unsorted endpoints, or inconsistent evidence.
    pub fn validate_for(
        &self,
        node_id: NodeId,
        ...
        if self.schema_version != 1 {
            return Err(ProtocolValidationError::new(
                ...
                "schemaVersion",
                "node heartbeat status schema is unsupported",
            ));
        }
        if self.node_id != node_id
            || self.heartbeat_generation != heartbeat_generation
            || self.controller_instance_id != controller_instance_id
        {
            return Err(ProtocolValidationError::new(
                ValidationCode::IdentityMismatch,
                "nodeId",
                "node heartbeat status identity binding is invalid",
            ));
        }
        if self.heartbeat_generation.get() == 0 {
            return Err(ProtocolValidationError::new(
                ValidationCode::OutOfRange,
                "heartbeatGeneration",
                "node heartbeat status generation must be positive",
            ));
        }
        if self.endpoints.len() > 16 {
            return Err(ProtocolValidationError::new(
                ValidationCode::OutOfRange,
                "endpoints",
                "node heartbeat status has too many endpoints",
            ));
        }
        if !self
            .endpoints
            ...
            .all(|pair| pair[0].endpoint_id < pair[1].endpoint_id)
        {
            return Err(ProtocolValidationError::new(
                ValidationCode::InconsistentState,
                "endpoints",
                "node heartbeat status endpoints must be uniquely sorted",
            ));
        }
...
/// Signed controller response returned after an accepted node heartbeat.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase", deny_unknown_fields)]
pub struct SignedNodeHeartbeatStatus {
    /// Complete status covered by the signature.
    pub document: NodeHeartbeatStatus,
    /// Ed25519 signature over the canonical status transcript.
...
mod tests {
    ...
    fn heartbeat_status() -> NodeHeartbeatStatus {
        let mut endpoints = vec![
            NodeEndpointStatus {
                endpoint_id: EndpointId::new(),
                readiness: EndpointReadiness::Pending,
                last_checked_at: None,
                error_code: None,
            },
            NodeEndpointStatus {
                endpoint_id: EndpointId::new(),
                readiness: EndpointReadiness::TcpUnreachable,
                last_checked_at: Some(timestamp("2026-07-11T20:00:00Z")),
                error_code: Some("direct_tcp_unreachable".to_string()),
            },
        ];
        ...
        NodeHeartbeatStatus {
            ...
            observed_at: timestamp("2026-07-11T20:00:01Z"),
    ...
    #[test]
    fn heartbeat_status_requires_exact_identity_and_canonical_endpoints() {
        let status = heartbeat_status();
        assert!(status
            .validate_for(
                status.node_id,
                status.heartbeat_generation,
        ...
        let mut reversed = status.clone();
        reversed.endpoints.reverse();
        assert!(reversed
            .validate_for(
                reversed.node_id,
                reversed.heartbeat_generation,
                reversed.controller_instance_id,
            )
            .is_err());

        let mut duplicate = status;
        duplicate.endpoints[1].endpoint_id = duplicate.endpoints[0].endpoint_id;
        assert!(duplicate
            .validate_for(
                duplicate.node_id,
                duplicate.heartbeat_generation,
                duplicate.controller_instance_id,
            )
            .is_err());
    }

    #[test]
    fn heartbeat_status_rejects_inconsistent_or_extended_evidence() {
        let mut status = heartbeat_status();
        status.endpoints[0].readiness = EndpointReadiness::TcpReachable;
        assert!(status
            .validate_for(
                status.node_id,
                status.heartbeat_generation,
        ...
        let mut value = serde_json::to_value(heartbeat_status()).unwrap();
        value["controllerDecision"] = serde_json::json!("trust-me");
        assert!(serde_json::from_value::<NodeHeartbeatStatus>(value).is_err());
    }
